...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
        message: `${name} has been added to Restaurants of ${createRestaurantAndAddToChainArray.name}`
      });
  } catch (err) {
    res
      .status(500)
      .json({
        message: "Something went wrong while adding a new Restaurant",
        err
      });
  }
});
...
router.get("/:restaurantId", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `${restaurant.name} has been found`, restaurant });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Something went wrong while finding a restaurant" });
  }
});
...
router.patch("/:restaurantId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        message: `${updatedRestaurant.name} was updated`,
        updatedRestaurant
      });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Something went wrong while updating a restaurant" });
  }
});
...
router.delete("/:restaurantId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Something went wrong while deleting a restaurant" });
  }
});
